...
def save_session(context):
    ...
    print("💾 Cookies saved (JSON)")

    # ✅ ALSO SAVE AS .txt (yt-dlp compatible)
    cookie_txt_path = COOKIE_JSON.replace(".json", ".txt")
    ...
    try:
        with open(cookie_txt_path, "w", encoding="utf-8") as f:
            ...
            for c in cookies:
                ...
                if expiry == -1:
                    expiry = int(time.time()) + 3600 * 24 * 365  # fallback 1 year

                # format: domain \t flag \t path \t secure \t expiry \t name \t value
                line = f"{domain}\tTRUE\t{path}\tFALSE\t{int(expiry)}\t{name}\t{value}\n"
                f.write(line)

        print("🍪 Cookies saved (TXT for yt-dlp)")
